<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>

<head lang="en">
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">

  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

  <link href="style-program.css" rel="stylesheet" type="text/css">
  <link href="animate.css" rel="stylesheet" type="text/css">
  <link href="landing.jsp" type="text/html">
  <link rel="shortcut icon" href="assets/Rooster_Browser_Tab_Icon.png">

  <title>Rooster - Home</title>
</head>

<body>
  <nav class="navbar">
    <ul>
      <li>
        <a href="#home">
          <img class="navbutton" src="assets/Home_Button.png" alt="Home">
        </a>
      </li>
      <li><a href="#projects"><img class="navbutton" src="assets/Projects_Button.png" alt="Projects"></a></li>
      <li><a href="#reminders"><img class="navbutton" src="assets/Reminders_Button.png" alt="Reminders"></a></li>

      <li class="rightnav"><a href="#help"><img src="assets/Help_Icon.png"></a></li>
      <li class="rightnav"><a href="#dark-mode"><img src="assets/Dark_Mode_Icon.png"></a></li>
      <li class="dropdown">
        <a href="javascript:void(0)" class="dropbtn"><img src="assets/Settings_Icon.png"></a>
        <div class="dropdown-content">
          <a href="#" class="dropdown-option">Change Name</a>
          <a href="#" class="dropdown-option">Change Profile Picture</a>
          <a href="#" class="dropdown-option">Change Background</a>
        </div>
      </li>
    </ul>
  </nav>
</body>

<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>

<h1><a href="landing.jsp">Landing link :)</a></h1>
<script type="text/javascript" src="js-landing.js"></script>

</html>
<!--<nav class="navbar">
        <ul>
            <li><a href="#home"><img class="navbutton" src="assets/Home_Button.png" alt="Home"></a></li>
            <li><a href="#projects"><img class="navbutton" src="assets/Projects_Button.png" alt="Projects"></a></li>
            <li><a href="#reminders"><img class="navbutton" src="assets/Reminders_Button.png" alt="Reminders"></a></li>
            <li class = "rightnav"><a href="#dark-mode">Dark Mode</a></li>
            <li class = "rightnav"><a href="#help">Help</a></li>
            <li class="dropdown">
              <a href="javascript:void(0)" class="dropbtn">Settings</a>
              <div class="dropdown-content">
                <a href="#">Change Name</a>
                <a href="#">Change Profile Picture</a>
                <a href="#">Change Background</a>
              </div>
            </li> 
          </ul>
    </nav>-->